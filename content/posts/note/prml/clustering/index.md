...
![K-Means 模型的聚类结果](assets/k-means/1-train.webp){width=500}

#### 1.2 一些优化

K-Means 模型的一个问题是，由于初始时的聚簇中心是随机选择的，如果选择得不好，可能会导致收敛到局部最优解而非全局最优解。例如对于同样的数据集，可能生成如下的聚类结果：

![一个不理想的聚类结果](assets/k-means/1-train-bad.webp){width=500}
...
![实验 1 - 训练集（指定划分成 3 类）](assets/k-means/1-train.webp){width=500}

![实验 1 - 测试集（指定划分成 3 类）](assets/k-means/1-test.webp){width=500}
...
![实验 2 - 训练集（指定划分成 5 类）](assets/k-means/2-train.webp){width=500}

![实验 2 - 测试集（指定划分成 5 类）](assets/k-means/2-test.webp){width=500}
...
![实验 3 - 训练集（指定划分成 3 类）](assets/k-means/3-train.webp){width=500}

![实验 3 - 测试集（指定划分成 3 类）](assets/k-means/3-test.webp){width=500}
...
![实验 4 - 训练集（指定划分成 3 类）](assets/k-means/4-train.webp){width=500}

![实验 4 - 测试集（指定划分成 3 类）](assets/k-means/4-test.webp){width=500}
...
![实验 5 - 训练集（指定划分成 3 类）](assets/k-means/5-train.webp){width=500}

![实验 5 - 测试集（指定划分成 3 类）](assets/k-means/5-test.webp){width=500}
...
![GMM 模型的聚类结果](assets/gmm/1-train.webp){width=500}
...
![实验 1 - 训练集（指定划分成 3 类）](assets/gmm/1-train.webp){width=500}

![实验 1 - 测试集（指定划分成 3 类）](assets/gmm/1-test.webp){width=500}
...
![实验 2 - 训练集（指定划分成 5 类）](assets/gmm/2-train.webp){width=500}

![实验 2 - 测试集（指定划分成 5 类）](assets/gmm/2-test.webp){width=500}
...
![实验 3 - 训练集（指定划分成 3 类）](assets/gmm/3-train.webp){width=500}

![实验 3 - 测试集（指定划分成 3 类）](assets/gmm/3-test.webp){width=500}
...
![实验 4 - 训练集（指定划分成 3 类）](assets/gmm/4-train.webp){width=500}

![实验 4 - 测试集（指定划分成 3 类）](assets/gmm/4-test.webp){width=500}
...
![实验 5 - 训练集（指定划分成 3 类）](assets/gmm/5-train.webp){width=500}

![实验 5 - 测试集（指定划分成 3 类）](assets/gmm/5-test.webp){width=500}
...
其中，$k$ 即选择的 $k$ 值，$W\_k$ 为数据集 $\mathbf{D}$ 聚类结果的簇内距离的平方和，${W'}\_k$ 为同规模均匀分布 $\mathbf{U}$ 聚类结果的簇内距离的平方和。这里 $W$ 的具体定义参见 [1.2](#1.2-一些优化) 节。
...
![实验 1 - 训练集 Gap-$k$ 图](assets/auto-k-means/1-gaps.webp){width=500}

模型选择的 $k$ 值为 $3$，聚类结果：

![实验 1 - 训练集（不指定 $k$ 值）](assets/auto-k-means/1-train.webp){width=500}

![实验 1 - 测试集（不指定 $k$ 值）](assets/auto-k-means/1-test.webp){width=500}
...
![实验 2 - 训练集 Gap-$k$ 图](assets/auto-k-means/2-gaps.webp){width=500}

模型选择的 $k$ 值为 $6$，聚类结果：

![实验 2 - 训练集（不指定 $k$ 值）](assets/auto-k-means/2-train.webp){width=500}

![实验 2 - 测试集（不指定 $k$ 值）](assets/auto-k-means/2-test.webp){width=500}
...
![实验 3 - 训练集 Gap-$k$ 图](assets/auto-k-means/3-gaps.webp){width=500}

模型选择的 $k$ 值为 $4$，聚类结果：

![实验 3 - 训练集（不指定 $k$ 值）](assets/auto-k-means/3-train.webp){width=500}

![实验 3 - 测试集（不指定 $k$ 值）](assets/auto-k-means/3-test.webp){width=500}
...
![实验 4 - 训练集 Gap-$k$ 图](assets/auto-k-means/4-gaps.webp){width=500}

模型选择的 $k$ 值为 $3$，聚类结果：

![实验 4 - 训练集（不指定 $k$ 值）](assets/auto-k-means/4-train.webp){width=500}

![实验 4 - 测试集（不指定 $k$ 值）](assets/auto-k-means/4-test.webp){width=500}
...
![实验 5 - 训练集 Gap-$k$ 图](assets/auto-k-means/5-gaps.webp){width=500}

模型选择的 $k$ 值为 $1$，聚类结果：

![实验 5 - 训练集（不指定 $k$ 值）](assets/auto-k-means/5-train.webp){width=500}

![实验 5 - 测试集（不指定 $k$ 值）](assets/auto-k-means/5-test.webp){width=500}
