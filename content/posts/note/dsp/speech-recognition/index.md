...
![signal - 语音波形图](assets/signal/time-domain.webp)

其短时过零率曲线如下：

![signal - 过零率](assets/signal/zcr.webp)
...
![signal - 语音波形图（17307130323）](assets/signal/17307130323-time-domain.webp)

![open - 语音波形图（17300200027）](assets/open/17300200027-time-domain.webp)

![print - 语音波形图（18307130365）](assets/print/18307130365-time-domain.webp)
...
![shop - 对数频谱图](assets/shop/spec-domain-16ms-hamming.webp)
...
![shop - 对数能量谱](assets/shop/energy-spec-16ms-hamming-filtered.webp)
...
![池化（Pooling）](assets/pooling.gif){width=500}

（图片来源：[_Comprehensive Guide to Different Pooling Layers in Deep Learning_][pooling-img-src]）
...
![一个典型的 CNN 网络](assets/cnn.webp)

（图片来源：[_A Comprehensive Guide to Convolutional Neural Networks — the ELI5 way_][cnn-img-src]）
...
![训练集 & 验证集的损失曲线](assets/loss.webp)

训练过程中的准确率曲线如下：

![准确率曲线](assets/accuracy.webp)

对于验证集中的数据，我们取得了 $94.36\\%$ 的准确率。对于测试集中的数据，我们取得了 $56.88\\%$ 的准确率，混淆矩阵如下：

![混淆矩阵](assets/confusion-matrix.webp){width=500}
