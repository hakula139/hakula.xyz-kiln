...
::: callout {type=tip title="解决方案"}
不要关闭弹出的「引数不正」错误提示，尝试在资源管理器里移动曲包目录（推荐使用二分法，即每次移动出错曲包目录里的一半）。如果某些目录无法被移动，那么出错的曲包就在其中。按 [#1.1](#1.1-部分曲包没有读取到) 的方案修改后，重复上述步骤直到所有目录均可正常移动。
:::

当然，置之不理也不影响正常游玩。可以等发现想玩的曲包没有正常读取时，再按 [#1.1](#1.1-部分曲包没有读取到) 进行操作。
...
最常见的情况是曲包目录名有问题，解决方案同 [#1.1](#1.1-部分曲包没有读取到)。
