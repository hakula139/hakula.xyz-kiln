...
featured_image = "/images/article-covers/74046731.webp"
tags = [
    "VPS",
    "Vagex",
    "挂机",
...
![IP Banned!](assets/vagex-mail.webp)
...
![Xshell 配置 - Connection > SSH > Tunneling](assets/xshell/tunneling.webp)

如图所示，设定 Listening Port 为客户端端口、Destination Port 为服务端端口。其中端口号为桌面号加 `5900`，例如 `5905` 端口即对应 `5` 号桌面。如果 VPS 设置了防火墙，记得放行相应端口。

![Xshell 配置 - Forwarding Rule](assets/xshell/forwarding-rule.webp)
...
![VNC Viewer 配置](assets/vnc-viewer-properties.webp)
...
![Firefox 设置 - History](assets/firefox/history.webp)

点击 Settings...。

![Firefox 设置 - Settings for Clearing History](assets/firefox/clear-history.webp)
...
![Firefox 设置 - Permissions](assets/firefox/permissions.webp)
...
![Vagex 插件 - Firefox Viewer 下载](assets/vagex/firefox-viewer-download.webp)

安装完插件后，点击插件按钮，输入 Vagex 账号密码登录。调整 Start with Firefox 选项为 true，使 Vagex 插件随 Firefox 自启动。此后点击 Start 启动插件，点击 Stop 停止插件。

![Vagex 插件 - Firefox Viewer 启动](assets/vagex/firefox-viewer-start.webp)
...
::: callout { type=success title="2019-04-14 可行方案" }
最终找到的方法是在手机上使用 Gmail 的 APP 注册，可以跳过手机验证，那接下来就没什么好讲的了。
:::

以下是我之前的折腾历程，都是些不可行的方案，就当排雷了。

::: callout { type=failure title="2019-04-14 不可行方案" open=false }
